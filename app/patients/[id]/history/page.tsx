...
export default async function HistoryPage({
  ...
  if (!entryId) {
    return (
      <main className="mx-auto min-h-screen max-w-4xl px-6 py-10">
        <p>Select an entry history from the patient timeline.</p>
  ...
  return (
    <main className="mx-auto min-h-screen max-w-4xl px-6 py-10">
      ...
      <p className="mt-1 text-stone-700">{entry.entry_type} · current v{entry.current_version}</p>
        ...
        {versions.map((version, index) => (
          <article className="rounded-md border border-stone-300 bg-white p-4" key={version.id}>
            <div className="flex flex-wrap items-center gap-2 text-sm text-stone-600">
              ...
              {version.reverted_from_version ? <span>from v{version.reverted_from_version}</span> : null}
            ...
            <p className="mt-2 text-sm text-stone-600">{diffSummary(versions[index + 1]?.content ?? null, version.content)}</p>
