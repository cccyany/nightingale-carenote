...
export async function POST(
  ...
  if (!body.success) return jsonError(400, "Invalid revert payload");
  ...
  if (data?.status === "conflict") return NextResponse.json(data, { status: 409 });
  if (data?.status !== "ok") return jsonError(404, "Entry or version not found");
