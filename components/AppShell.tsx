    ...
    <div className="min-h-screen bg-[#f5f7f4] text-stone-950">
      <header className="sticky top-0 z-20 border-b border-stone-200 bg-white/95 backdrop-blur">
        <div className="mx-auto flex max-w-7xl flex-wrap items-center justify-between gap-3 px-4 py-3 sm:px-6 lg:px-8">
          <div>
            <Link className="text-lg font-semibold tracking-tight text-stone-950" href={demo ? `/patients?demo=${encodeURIComponent(demo)}` : "/"}>
          ...
          <nav className="flex flex-wrap items-center gap-2 text-sm" aria-label="Primary navigation">
            <Link className="rounded-md px-3 py-2 text-stone-700 hover:bg-stone-100 focus:outline-none focus:ring-2 focus:ring-teal-600" href={demo ? `/patients?demo=${encodeURIComponent(demo)}` : "/login"}>
              Patients
            </Link>
            {patientId && demo ? (
              <Link className="rounded-md px-3 py-2 text-stone-700 hover:bg-stone-100 focus:outline-none focus:ring-2 focus:ring-teal-600" href={`/patients/${patientId}?demo=${encodeURIComponent(demo)}`}>
